  ...
  try {
    ...
    saveBtn.onclick = () => {
      ...
      for (const action in inputs) {
        let val = inputs[action].value.trim()
        if (val === "Space") val = " "
        if (val && used[val]) {
          alert(
            `Duplicate key "${
      ...
      for (const action in inputs) {
        let val = inputs[action].value.trim()
        if (val === "Space") val = " "
        localStorage.setItem(storageKeys[action], val || "")
        savedKeys[action] = val
      }
      ...
      function globalHandler(e) {
        ...
        for (const action in savedKeys) {
          ...
          if ((isSpace && saved === " ") || key === saved.toUpperCase()) {
            ...
            // First, try to find the emoji button directly in the DOM
            const existingBtn = Array.from(
              document.querySelectorAll("button[aria-label]")
            ).find((b) => b.getAttribute("aria-label")?.includes(target))

            if (existingBtn) {
              // Emoji button already visible, click it directly
              existingBtn.click()
              return
            }

            // If not found, open the react panel and then select the emoji
            reactBtn?.click?.()
